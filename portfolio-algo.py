...
tickerSymbols = ['GOOGL', 'AAPL', 'MSFT', 'SAR', 'UNH', 'WMT', 'NSRGY', 'GOLD', 'BTC-USD', 'ETH-USD']
stock_prices_df = pd.DataFrame()
for tickerSymbol in tickerSymbols:
  ticker = yf.Ticker(tickerSymbol)
  tickerDf = ticker.history(period='1d', start = '2010-1-1', end='2022-2-26')
  tickerDf = tickerDf.drop(columns=['Open', 'High', 'Low', 'Dividends', 'Stock Splits'])
  if stock_prices_df.index.empty:
    stock_prices_df = pd.DataFrame(index=tickerDf.index)
  stock_prices_df[tickerSymbol] = tickerDf['Close'] #Adding daily closing prices
#display(stock_prices_df)
#stock_prices_df.to_csv('result.csv', index = True, header=True) #Download new csv

mu = expected_returns.mean_historical_return(stock_prices_df) #expected returns
...
ax.scatter(risk_std_tangent, risk_ret_tangent, marker="*", s=100, c="r", label="Risk efficient")
plt.savefig('graph.png') #save image locally

latest_prices = get_latest_prices(stock_prices_df)

da = DiscreteAllocation(weights, latest_prices, total_portfolio_value=100000) #change portfolio value
