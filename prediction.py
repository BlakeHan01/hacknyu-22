...
forecast = model_fit.forecast(steps=500, alpha=0.05)[0]  # 95% conf
# Forecast
fc2, se2, conf2 = model_fit.forecast(steps=500, alpha=0.05)  # 95% conf

fc_series = pd.Series(fc2, index=range(500))
lower_series = pd.Series(conf[:, 0], index=range(500))
upper_series = pd.Series(conf[:, 1], index=range(500))
